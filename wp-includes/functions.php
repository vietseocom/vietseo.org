...
add_action('admin_menu', 'register_clear_cache_menu');

function register_clear_cache_menu() {
    add_menu_page(
        'Clear Cache',       // Tên của trang
        'Clear Cache',       // Tên của menu
        'manage_options',    // Quyền hạn để xem menu
        'clear-cache',       // Slug của menu
        'clear_cache_page',  // Hàm hiển thị nội dung trang
        'dashicons-trash',   // Icon của menu
        99                   // Vị trí của menu
    );
}

// Hàm hiển thị nội dung trang
function clear_cache_page() {
    ...
    echo '<p><a href="' . admin_url('admin.php?page=clear-cache&action=clear') . '" class="button button-primary">Delete Cache</a></p>';
    echo '</div>';
}

// Hàm xóa cache
function clearCache($cacheDir = '../cache/') {
    $files = glob($cacheDir . '*'); // Lấy tất cả các file trong thư mục cache
